...
- Why is a horizontal bar chart better than a scatter plot for this data?
  - Because it is easier to read and compare the values of the bars.
- When would you choose a vertical bar chart over horizontal?
  - when the names for the columns are longer.

**Question 2: Aesthetic Mappings**
- What does the color highlighting accomplish in Approach 4?
  - It helps to highlight the Colorado Rockies as the most run-friendly ballpark in 2010.
- How does position (x/y) compare to color for encoding data?
  - Position is better for encoding data because it is easier to read and compare the values of the bars.
:::
...
colorado_index = avgDF_2010_sorted[avgDF_2010_sorted.home.reset_index().home == "COL"].index[0]
...
- What elements make this visualization suitable for a business presentation?
  - The suitable elements are the title, labels, and the legend.
- Is the annotation on the visualization helpful?  Can you fix its placement?
  - The annotation is helpful, but the placement is not optimal.
    the index had to be reset to get the correct index.
:::
...
# Sort data for each year separately for better visualization
comparison_data_2010_sorted = comparison_data.sort_values('runs_2010', ascending=True)
comparison_data_2021_sorted = comparison_data.sort_values('runs_2021', ascending=True)

# Create color arrays for each sorted dataset
...
axes[0].set_ylabel("Stadium")
axes[0].set_xlabel("Average Runs Per Game")
axes[0].barh(comparison_data_2010_sorted.home, comparison_data_2010_sorted.runs_2010, color=colorado_colors_2010)
axes[0].set_title("Most Run-Friendly Ballpark in 2010", 
             fontsize=16, fontweight='bold', pad=20)

axes[1].barh(comparison_data_2021_sorted.home, comparison_data_2021_sorted.runs_2021, color=colorado_colors_2021)
axes[1].set_ylabel("Stadium")
axes[1].set_xlabel("Average Runs Per Game")
axes[1].set_title("Most Run-Friendly Ballpark in 2021", 
             fontsize=16, fontweight='bold', pad=20)
...
axes[1].legend(handles=[colorado_bar, other_bar], loc='lower right', frameon=True)  
...
The two-facet visualization presents a comparative analysis of total runs per stadium across the 2010 and 2021 baseball seasons. Each subplot displays a horizontal bar chart where stadiums are mapped to the y-axis and average runs per game to the x-axis, enabling easy comparison across all Major League Baseball venues. Colorado (COL) is highlighted in darkorchid to distinguish it from other stadiums, which appear in light grey. This visual encoding allows viewers to quickly identify not just Colorado's absolute performance, but also its relative standing within the league over time. The juxtaposition of the two years provides insight into whether stadium effects have remained consistent or changed over the decade, with Colorado's reputation for being run-friendly due to its high altitude (5,200 feet above sea level) serving as the focal point of investigation. The consistent color highlighting across facets maintains visual continuity, while the sorted data makes it easy to identify which stadiums rank highest in run production for each respective season.

- Does AI come to the right conclusion?  If not, why not?
  - No the AI does not come to the right conlusion because in 2021 Colarado was not the most run friendly park.
:::
